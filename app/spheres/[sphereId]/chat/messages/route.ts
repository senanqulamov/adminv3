...
export async function GET(req: NextRequest, ctx: { params: Promise<{ sphereId: string }> }) {
  const { params } = ctx
  const { sphereId } = await params
  const { searchParams } = new URL(req.url)
  const cursor = searchParams.get("cursor")
  const limit = Number(searchParams.get("limit") ?? 30)
  const out = listMessages(sphereId, null, cursor, limit)
  return NextResponse.json(out)
}

export async function POST(req: NextRequest, ctx: { params: Promise<{ sphereId: string }> }) {
